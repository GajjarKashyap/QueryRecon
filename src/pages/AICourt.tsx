...
import { AIThinkingIndicator } from '../components/assistant/AIThinkingIndicator';
...
function TurnView({ turn }: { turn: AICourtTurn }) {
  return (
    <article className="space-y-4 border-b border-border/70 pb-8 last:border-0">
      ...
      {turn.status === 'running' && (
        <AIThinkingIndicator label={turn.progress || 'Opening the case…'} detail="Reviewing evidence · comparing claims · preparing the ruling" />
      )}
      ...
      {turn.format !== 'solo' && turn.opinions.length > 0 && <div className="grid gap-4 xl:grid-cols-2"><ProviderOpinion response={turn.opinions[0]} title={`${turn.opinions[0]?.provider || 'First model'} opinion`} /><ProviderOpinion response={turn.opinions[1]} title={`${turn.opinions[1]?.provider || 'Second model'} opinion`} /></div>}
...
export default function AICourt() {
  ...
  const [format, setFormat] = useState<'solo' | 'court'>('court');
  ...
  const activeParticipants = format === 'solo' ? participants.slice(0, 1) : participants;
  const canRun = Boolean(question.trim() && activeParticipants.every(item => keys[item.provider] && item.model.trim()) && !busy);
  const totalCalls = format === 'solo' ? 1 : mode === 'god' ? 5 : 3;
  const latestUsage = useMemo(() => activeCase?.turns.reduce((sum, turn) => sum + (turn.usage?.totalTokens || 0), 0) || 0, [activeCase]);

  const detectModels = async () => {
    const selectedProviders = [...new Set(activeParticipants.map(item => item.provider))];
    if (selectedProviders.some(provider => !keys[provider])) { toast('Save an API key for every selected provider in Settings first.', 'error'); return; }
    setDetecting(true);
  ...
  const submit = async () => {
    ...
    const activeJudge = format === 'solo' ? 0 : judge;
    const pending: AICourtTurn = { id: turnId, question: question.trim(), format, mode, judge: participants[activeJudge].provider, status: 'running', progress: format === 'solo' ? 'Opening solo analysis' : 'Opening the case', createdAt: now, opinions: [], reviews: [] };
    ...
    try {
      ...
      const result = await runAICourt({
        question: prompt, history, mode: format === 'solo' ? 'standard' : mode, judge: activeJudge,
        participants: activeParticipants.map(item => ({ ...item, apiKey: keys[item.provider] })),
      ...
      <div className="mx-auto grid max-w-[100rem] gap-6 xl:grid-cols-[17rem_minmax(0,1fr)]">
        ...
        <section className="min-w-0 space-y-6">
          <header className="border-b border-border/70 pb-6">
            <div className="mb-3 flex items-center gap-2 text-xs font-medium tracking-[0.16em] text-primary uppercase"><Scale className="h-4 w-4" /> Solo analysis or multi-model deliberation</div>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><h1 className="text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">AI Court</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Use one AI for a fast single answer, or bring two models into court for independent opinions, cross-examination, and a saved ruling.</p></div>{latestUsage > 0 && <Badge variant="outline">Case usage · {latestUsage.toLocaleString()} tokens</Badge>}</div>
          </header>

          <Card className="gap-4 border-border/80 bg-surface p-5">
            <div className="grid grid-cols-2 gap-2 rounded-xl border border-border/70 bg-background/50 p-1.5" aria-label="AI Court format"><button type="button" aria-pressed={format === 'solo'} onClick={() => { setFormat('solo'); setJudge(0); }} className={`rounded-lg px-3 py-2.5 text-left transition-[background-color,color,transform] duration-150 active:scale-[0.98] ${format === 'solo' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-surface hover:text-foreground'}`}><span className="block text-sm font-semibold">Solo AI</span><span className="block text-[0.68rem] opacity-75">One model · one paid call</span></button><button type="button" aria-pressed={format === 'court'} onClick={() => setFormat('court')} className={`rounded-lg px-3 py-2.5 text-left transition-[background-color,color,transform] duration-150 active:scale-[0.98] ${format === 'court' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-surface hover:text-foreground'}`}><span className="block text-sm font-semibold">AI Court</span><span className="block text-[0.68rem] opacity-75">Two models · consensus</span></button></div>
            <div className={`grid gap-4 ${format === 'court' ? 'lg:grid-cols-2' : ''}`}>
              {activeParticipants.map((participant, rawIndex) => {
            ...
            <div className={`grid gap-4 ${format === 'court' ? 'sm:grid-cols-[1fr_auto]' : ''}`}>
              {format === 'court' && <label className="grid gap-2 text-xs font-medium">Final judge<Select value={String(judge)} onValueChange={value => setJudge(Number(value) as 0 | 1)}><SelectTrigger className="h-12 w-full border-border bg-background px-3 shadow-sm"><SelectValue>{`Model ${judge + 1} · ${PROVIDERS.find(item => item.id === participants[judge].provider)?.label}`}</SelectValue></SelectTrigger><SelectContent align="start" className="p-1.5 shadow-xl"><SelectItem value="0" className="min-h-11 px-3">Model 1 · {PROVIDERS.find(item => item.id === participants[0].provider)?.label}</SelectItem><SelectItem value="1" className="min-h-11 px-3">Model 2 · {PROVIDERS.find(item => item.id === participants[1].provider)?.label}</SelectItem></SelectContent></Select></label>}
            ...
            {format === 'court' && <div className={`flex flex-col gap-3 rounded-xl border p-4 sm:flex-row sm:items-center sm:justify-between ${mode === 'god' ? 'border-warning/35 bg-warning/5' : 'border-border/70 bg-background/25'}`}>
            ...
            {activeParticipants.some(item => !keys[item.provider]) && <div className="flex items-center gap-2 rounded-lg border border-warning/30 bg-warning/5 px-3 py-2 text-xs text-warning"><AlertTriangle className="h-4 w-4" /> Save API keys for the selected providers in Settings. Keys stay in this browser and are never added to Git.</div>}
          </Card>
          ...
          <Card className="min-h-[35rem] gap-0 border-border/80 bg-surface">
            ...
            <form onSubmit={event => { event.preventDefault(); void submit(); }} className="sticky bottom-0 border-t border-border/80 bg-background/90 p-4 backdrop-blur-xl">
              <div className="flex items-end gap-3"><textarea value={question} onChange={event => setQuestion(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void submit(); } }} rows={3} placeholder={format === 'solo' ? `Ask ${participants[0].provider}…` : `Ask ${participants[0].provider} and ${participants[1].provider} to decide…`} disabled={busy} className="min-h-16 flex-1 resize-none rounded-xl border border-border bg-background px-3.5 py-3 text-sm outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/15 disabled:opacity-60" /><Button type="submit" size="icon-lg" className="h-12 w-12 rounded-xl" disabled={!canRun} aria-label="Submit to AI Court">{busy ? <BrainCircuit className="ai-button-thinking" /> : mode === 'god' && format === 'court' ? <Sparkles /> : <Send />}</Button></div>
