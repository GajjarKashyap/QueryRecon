...
Amazon Bedrock is available as a Hermes inference route in the browser workspace, first-run setup, and floating assistant. Select **Amazon Bedrock**, detect the models enabled for the Hermes AWS account, or enter any exact Bedrock model ID manually. In **Settings → Local AI Assistant → Hermes Agent**, enter the Bedrock long-term API key and AWS region, then choose **Save & restart**. The local-only setup bridge writes `AWS_BEARER_TOKEN_BEDROCK` and `AWS_REGION` to Hermes' `.env`; the Bedrock key is never persisted in browser storage or committed to this repository.
